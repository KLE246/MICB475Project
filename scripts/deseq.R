...
combined_deseq <- phyloseq_to_deseq2(combined_plus1, ~ cohort) # anemia vs infant
...
vol_plot <- res %>%
  ...
  labs(x = "USA/Peru change", title="Significant abundance differences between infants in USA and Peru")

vol_plot
ggsave(filename="plots/vol_plot_deseq_cohort.png",vol_plot)
...
bar_plot <- ggplot(sigASVs) +
  ...
  labs(y = "USA/Peru change", title="Change in Genus abundance between infants in USA and Peru")+
  theme(legend.position="none")

bar_plot
ggsave(filename="plots/bar_plot_deseq_cohort.png",bar_plot)
